...
# Streamlit computa las 4 pestañas en cada rerun aunque solo una esté visible
# — medido en 2,37 s por rerun con caché caliente, la más cara de las 15 páginas.
...
if tab_dim.open:
    with tab_dim:
        ...
        c3.metric(
            "Líneas por posición ocupada", f"{lineas_alt / ocupadas:.2f}" if ocupadas else "—"
        )
        c4.metric(
            "Son clase A o B",
        ...
        st.caption(
            ...
            "crítico que el propio plan señala.".format(
                f"{heredadas:,}".replace(",", "."),
                heredadas / lineas_alt * 100 if lineas_alt else 0,
            )
        )
    ...
    with tab_reg:
        ...
        if subidas:
            for archivo in subidas:
                ...
                en_sistema = sum(
                    ...
                    for u, i in zip(
                        revision["ubicacion"], revision["id_especificacion"], strict=True
                    )
                )
        ...
        if conteos.empty:
            ...
            if "estado_ciclo" in conteos.columns and conteos["estado_ciclo"].notna().any():
                ...
                abierto = conteos[
                    conteos["estado_ciclo"].isin(
                        [ESTADO_CONTEO_PENDIENTE, ESTADO_CONTEO_CONFIRMADO]
                    )
                ]
                resumen = (
                    ...
                    if len(abierto):
                        ...
                        st.dataframe(
                            ...
                            column_config={
                                ...
                                "estado_ciclo": st.column_config.TextColumn(
                                    "Estado", width="medium"
                                ),
            ...
            if len(posiciones) and posiciones["dias_desde_conteo"].notna().any():
                ...
                a2.metric(
                    "Antigüedad mediana", f"{contadas['dias_desde_conteo'].median():.0f} días"
                )
                if len(clase_a):
                    ...
                    a3.metric(
                        ...
                        delta_color="normal"
                        if media_a < META_ANTIGUEDAD_CONTEO_DIAS
                        else "inverse",
                    )
                st.caption(
            ...
            malas = conteos[conteos["exacta"] == 0].sort_values(
...
if tab_pick.open:
    with tab_pick:
        ...
        if picking.empty:
            ...
            por_posicion = picking.groupby(
                ["ubicacion", "rack", "clase_posicion"], as_index=False
            ).agg(lineas=("id_especificacion", "count"), valor=("valor_linea", "sum"))
            ...
            for clase, cuota in CUOTA.items():
                ...
                if n:
                    partes.append(
                        estrato.sample(
                            n=n, random_state=int(ciclo) * 100 + ORDEN_CLASE.index(clase)
                        )
                    )
            muestra = (
