...
# Streamlit computa las 2 pestañas en cada rerun aunque solo una esté visible
# — medido en 1,53 s por rerun con caché caliente.
tab_pago, tab_cancel = st.tabs(["Auditoría de pago", "Cancelaciones e impagos"], on_change="rerun")
...
if tab_pago.open:
    with tab_pago:
        ...
        if not estado_pago.empty:
            ...
            if len(canales):
                linea = " · ".join(
                    f"**{formato_miles(v)}** {k.lower()}" for k, v in canales.items()
                )
                st.caption(
        ...
        if pagos.empty:
            ...
            if len(con_ver):
                ...
                reintentos = (
                    rechazados["comprobantes"].median() if len(rechazados) else float("nan")
                )
            ...
            if len(pendientes):
                ...
                st.warning(
                    ...
                    + (
                        f", de los cuales **{len(viejos)} llevan más de 24 h**."
                        if len(viejos)
                        else "."
                    )
